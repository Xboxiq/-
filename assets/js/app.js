...
/* ---------- عزل الطباعة — ورقة النموذج فقط (بلا عناصر الواجهة) ---------- */
function isolatedPrint(mode) {
  const formVariant = (mode === 'form' && App.current) ? getFormMode(App.current) : 'original';
  const cls = mode === 'preview' ? 'print-preview' : ('print-form print-form--' + formVariant);
  document.body.classList.add(...cls.split(/\s+/));
  const cleanup = () => {
    cls.split(/\s+/).forEach((c) => document.body.classList.remove(c));
    window.removeEventListener('afterprint', cleanup);
  };
  window.addEventListener('afterprint', cleanup);
  setTimeout(() => { if (document.body.classList.contains(cls)) cleanup(); }, 60000);
